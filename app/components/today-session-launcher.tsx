...
export default function TodaySessionLauncher({ profileId, profileName, appointmentId = null, notify, onExit, onFinished }: {
  ...
  function retry() { setError(""); setStartError(""); setLoading(true); setPreparation(null); setRevision((current) => current + 1); }
  async function start() {
    if (!preparation || loading || error || startLock.current) return;
    startLock.current = true; setStarting(true); setStartError("");
    try {
      ...
      const message = cause instanceof Error ? cause.message : "No se pudo iniciar la sesión.";
  ...
  return <RealSessionSetup preparation={preparation} loading={loading} value={value} starting={starting} error={error} actionError={startError} onRetry={retry} profileOptions={[{ id: profileId, label: profileName }]} onChange={(next) => { setStartError(""); setValue(next); }} onClose={() => { if (!startLock.current) onExit(); }} onStart={start}/>;
}
