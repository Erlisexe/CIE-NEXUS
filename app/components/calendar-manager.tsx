...
import { clinicalFilterParams, matchesClinicalFilter, profileMatchesClinicalFilter, upcomingCalendarPeriod } from "../../lib/clinical-filter";
...
export default function CalendarManager({
  ...
  const filter = { site: selectedSite, profileId: selectedProfileId };
  const query = clinicalFilterParams(filter);
  query.set("from", from); query.set("to", to); query.set("upcoming", "1");
  const requestUrl = `/api/calendar?${query}`;
  const pending = loading || loadedRequest !== requestUrl;
  ...
  useEffect(() => {
    ...
    clientRequest(requestUrl, { cache: "no-store", signal: controller.signal })
      .then(async (response) => ({ response, data: await response.json() as { appointments?: CalendarAppointment[]; upcomingAppointments?: CalendarAppointment[]; professionals?: ClinicalProfessional[]; canManage?: boolean; error?: string } }))
      .then(({ response, data }) => {
        if (cancelled) return;
        if (!response.ok) throw new Error(data.error || "No se pudo cargar el calendario.");
        setAppointments(data.appointments || []);
        setUpcomingAppointments(data.upcomingAppointments || data.appointments || []);
        setProfessionals(data.professionals || []);
        setCanManage(Boolean(data.canManage));
      })
      .catch((error: Error) => { if (!cancelled) { setLoadError(error.message); notify(error.message); } })
      .finally(() => { if (!cancelled) { setLoadedRequest(requestUrl); setLoading(false); } });
    return () => { cancelled = true; controller.abort(); };
  // The date range is the source of truth; toast identity does not change it.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [requestUrl, reloadRevision]);

  useEffect(() => {
    // A form opened in the previous scope must not remain actionable.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setDraft(null); setSelectedAppointment(null); setCancelTarget(null); setDeleteTarget(null);
  }, [selectedSite, selectedProfileId]);

  const activeProfiles = profiles.filter((profile) => profile.status === "active" && profileMatchesClinicalFilter(profile, filter));
  const matchesAppointment = (appointment: CalendarAppointment) => matchesClinicalFilter({ profileId: appointment.profileId, site: appointment.profileSite || profiles.find((profile) => profile.id === appointment.profileId)?.site || appointment.site }, filter);
  const visibleAppointments = pending || loadError ? [] : appointments.filter(matchesAppointment);
  ...
  const upcomingPeriod = upcomingCalendarPeriod(today);
  const allUpcoming = pending || loadError ? [] : upcomingAppointments.filter((appointment) => matchesAppointment(appointment) && (appointment.status === "scheduled" || appointment.status === "in_progress") && appointment.sessionDate >= upcomingPeriod.from && appointment.sessionDate <= upcomingPeriod.to);
  const upcoming = allUpcoming.slice(0, 8);

  function openNew(date = today) {
    if (!activeProfiles.length) { notify("No hay niños activos en este filtro. Limpia o cambia el filtro para programar."); return; }
  ...
  async function save() {
    if (!draft) return;
    if (!activeProfiles.some((profile) => profile.id === draft.profileId)) return notify("Selecciona un niño dentro del filtro activo.");
    setSaving(true);
    try {
      const response = await clientRequest("/api/calendar", { method: draft.id ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...draft, viewFilter: filter }) });
  ...
  return <div className={`calendar-module ${compact ? "compact" : ""}`}>
    ...
    <section className="formation-panel calendar-board">
      ...
      {!pending && loadError ? <div className="load-error" role="alert"><p>{loadError}</p><button onClick={load}>Reintentar carga</button></div> : pending ? <div className="calendar-loading" role="status"><LoaderCircle className="spin" size={25}/><strong>Cargando agenda…</strong></div> : <div className={`calendar-grid ${view}`}>
        {WEEKDAYS.map((weekday) => <span className="calendar-weekday" key={weekday}>{weekday}</span>)}
        {days.map((day) => {
          const dayIso = iso(day);
          const dayAppointments = visibleAppointments.filter((appointment) => appointment.sessionDate === dayIso);
