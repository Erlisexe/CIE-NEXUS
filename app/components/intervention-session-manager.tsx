...
type Measurement = "percentage" | "frequency" | "duration" | "latency" | "occurrence";
...
function stageDestination(stage: CriterionStage) {
  return stage === "baseline" ? "Cerrado o Adquisición" : stage === "acquisition" ? "Masterizado" : stage === "generalization" ? "Generalizado" : "Cerrado";
}
...
function unitFor(measurement: Measurement) {
  return measurement === "percentage" || measurement === "occurrence" ? "%" : measurement === "frequency" ? "ocurrencias" : "segundos";
}

function defaultCriterion(stage: CriterionStage, measurement: Measurement = "percentage"): Criterion {
  return {
    metric: measurement === "percentage" || measurement === "occurrence" ? "percentage_correct" : "value",
    operator: "gte",
    threshold: stage === "baseline" ? 90 : 80,
    requiredSessions: stage === "baseline" ? 1 : stage === "generalization" || stage === "maintenance" ? 2 : 3,
    minTrials: measurement === "percentage" || measurement === "occurrence" ? 3 : 1,
    consecutive: true,
...
function blankProgram(profile?: PersonnelProfile): ProgramForm {
  return {
    ...
    objective: "",
    instructions: "",
    graphConfig: { graphType: "line", designType: "AB", primaryTargetId: null, primaryTargetIndex: 0, showPoints: true, showLegend: true },
...
export default function InterventionSessionManager({
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch(mode === "sessions" && historyOnly && selectedProfileId !== "all"
        ? `/api/intervention-programs?profileId=${encodeURIComponent(selectedProfileId)}`
        : mode === "sessions" ? "/api/intervention-programs?scope=mine" : "/api/intervention-programs"),
      mode === "sessions" && !historyOnly ? fetch(`/api/calendar?from=${new Date().toISOString().slice(0, 10)}&mine=1`) : Promise.resolve(null),
      mode === "sessions" ? fetch("/api/session-note-templates", { cache: "no-store" }) : Promise.resolve(null),
    ])
      .then(async ([response, calendarResponse, templateResponse]) => {
        ...
        if (!cancelled) {
          ...
          if (mode === "sessions" && initialAppointment) {
            setSetup({ profileId: initialAppointment.profileId, sessionDate: initialAppointment.sessionDate, context: initialAppointment.sessionType || "Terapia individual", noteTemplateId: templateData?.templates?.[0]?.id || DEFAULT_SESSION_NOTE_TEMPLATE.id });
  ...
  }, [mode, historyOnly, selectedProfileId, initialAppointment, onAppointmentConsumed]);
  ...
  const activePrograms = programs.filter((program) => program.status === "active" && Boolean(program.profileId && activeProfileIds.has(program.profileId)) && (selectedProfileId !== "all" ? program.profileId === selectedProfileId : selectedSite === "Todas" || program.site === selectedSite));
  const allTargets = activePrograms.flatMap((program) => program.targets);
  const visibleProgramIds = new Set(activePrograms.map((program) => program.id));
  const visibleSessions = sessions.filter((session) => visibleProgramIds.has(session.programId));
  const visibleSessionGroups = groupProgramSessions(visibleSessions);
  ...
  async function saveProgram() {
    ...
    setSaving(true);
    try {
      const submit = (confirmMasteryRecalculation = false) => fetch("/api/intervention-programs", {
  ...
  function openSessionSetup(programId?: string) {
    const selectedProgram = programs.find((program) => program.id === programId)
      ...
      || activePrograms[0];
    setSetup({ profileId: selectedProgram?.profileId || "", sessionDate: new Date().toISOString().slice(0, 10), context: "", noteTemplateId: noteTemplates[0]?.id || DEFAULT_SESSION_NOTE_TEMPLATE.id });
  ...
  function openScheduledSession(appointment: CalendarAppointment) {
    ...
    setSetup({ profileId: appointment.profileId, sessionDate: appointment.sessionDate, context: appointment.sessionType || "Terapia individual", noteTemplateId: noteTemplates[0]?.id || DEFAULT_SESSION_NOTE_TEMPLATE.id });
  ...
  async function saveNoteTemplate() {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/session-note-templates", {
...
function ProgramModal({
  ...
  return <div className="modal-backdrop program-modal-backdrop"><section className="intervention-program-modal" role="dialog" aria-modal="true" aria-labelledby="program-modal-title"><div className="modal-title sticky-modal-title"><div><p className="section-kicker">Programa de intervención</p><h2 id="program-modal-title">{form.id ? "Editar programa y criterios" : "Crear programa medible"}</h2></div><button aria-label="Cerrar" onClick={onClose}><X size={19}/></button></div><div className="program-modal-body">
    ...
    <section className="program-graph-config"><div className="modal-section-heading"><span>2</span><div><strong>Visualización principal</strong><p>Define la gráfica que se mostrará primero al revisar este programa.</p></div></div><div className="program-form-grid"><label><span>Tipo de gráfica</span><select value={form.graphConfig.graphType} onChange={(event) => onChange({ ...form, graphConfig: { ...form.graphConfig, graphType: event.target.value as ProgramGraphType } })}><option value="line">Línea</option><option value="bar">Barras</option><option value="cumulative">Acumulativa</option></select></label>{form.graphConfig.graphType === "line" && <label><span>Diseño o fases</span><select value={form.graphConfig.designType} onChange={(event) => onChange({ ...form, graphConfig: { ...form.graphConfig, designType: event.target.value as ProgramGraphDesign } })}><option value="AB">AB</option><option value="ABA">ABA</option><option value="ABAB">ABAB</option><option value="BAB">BAB</option><option value="multiple-baseline">Línea base múltiple</option><option value="multielement">Multielemento</option><option value="changing-criterion">Criterio cambiante</option><option value="custom">Fases personalizadas</option></select></label>}<label><span>Target principal</span><select value={selectedGraphTargetIndex} onChange={(event) => { const index = Number(event.target.value); onChange({ ...form, graphConfig: { ...form.graphConfig, primaryTargetIndex: index, primaryTargetId: form.targets[index]?.id || null } }); }}>{form.targets.map((target, index) => <option value={index} key={target.id || index}>{target.code} · {target.name || `Target ${index + 1}`}</option>)}</select></label><label className="graph-config-toggles"><span>Presentación</span><span><input type="checkbox" checked={form.graphConfig.showPoints} onChange={(event) => onChange({ ...form, graphConfig: { ...form.graphConfig, showPoints: event.target.checked } })}/> Mostrar puntos</span><span><input type="checkbox" checked={form.graphConfig.showLegend} onChange={(event) => onChange({ ...form, graphConfig: { ...form.graphConfig, showLegend: event.target.checked } })}/> Mostrar leyenda</span></label></div></section>
