...
export default function InterventionSessionManager({
  ...
  useEffect(() => {
    ...
    fetch(`/api/session-collection?${query.toString()}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [realSetupOpen, realSetup.profileId, linkedAppointmentId]);
  ...
  async function startRealSession() {
    ...
    try {
      const response = await fetch("/api/session-collection", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return <>
    ...
    {realSetupOpen && <RealSessionSetup preparation={realPreparation} loading={realPreparationLoading} value={realSetup} starting={realStarting} profileOptions={sessionProfileOptions} onChange={(value) => { if (value.profileId !== realSetup.profileId) { setRealPreparation(null); setRealPreparationLoading(true); } setRealSetup(value); }} onClose={() => { setRealSetupOpen(false); setRealPreparation(null); setRealPreparationLoading(false); setLinkedAppointmentId(null); }} onStart={startRealSession}/>}
