...
import { isBinaryOpportunityMeasurement, measurementDisplayLabel, normalizeMeasurementConfig, usesEventCount, usesPartialIntervals, usesPromptLevels, usesTaskAnalysis, usesTimedMeasurement } from "../../lib/clinical-measurement";
...
function responseLabel(target: Pick<CollectionTarget, "measurement" | "measurementDimension" | "recordingFormat">, code: TrialResponseCode | null | undefined) {
  // Historic occurrence rows used the old I/X storage convention.  Present
  // those records as occurrence data as well, without rewriting evidence.
  if (!usesPromptLevels(target) && (code === "O" || code === "I")) return "Ocurrió";
  if (!usesPromptLevels(target) && (code === "N" || code === "X")) return "No ocurrió";
  return TRIAL_RESPONSES.find((item) => item.code === code)?.label || "Sin registro previo";
}

function responseGlyph(target: Pick<CollectionTarget, "measurement" | "measurementDimension" | "recordingFormat">, code: TrialResponseCode | null | undefined, value: number) {
  if (!usesPromptLevels(target)) return code === "O" || code === "I" || (!code && value > 0) ? "O" : "N";
  return code || (value > 0 ? "I" : "X");
}

function measurementLabel(target: Pick<CollectionTarget, "measurement" | "measurementDimension" | "recordingFormat">) {
  return measurementDisplayLabel(target);
}
...
function resultText(target: CollectionTarget, capture?: TargetCapture) {
  ...
  if (isDiscrete(target)) return `${result.correct || 0}/${result.opportunities} · ${result.value}%`;
  if (usesEventCount(target)) return `${result.value} ocurrencias${result.ratePerMinute !== null && result.ratePerMinute !== undefined ? ` · ${result.ratePerMinute}/min` : ""}`;
...
function RealSheetRow({ target, capture, paused, onOpen, onTrial, onFrequency }: {
  ...
  const promptLevels = usesPromptLevels(target);
  const quickCodes: TrialResponseCode[] = promptLevels ? (probe ? ["I", "X"] : ["I", "G", "V", "X"]) : ["O", "N"];
  return <article className={`${status.minimumMet ? status.evaluation.status === "met" ? "sample-met" : "sample-ready" : ""}`}>
    <button className="real-sheet-summary" onClick={onOpen}>
      ...
      <strong>{target.code} · {target.name}</strong><span>{responseLabel(target, last)}</span><em>{resultText(target, capture)}</em><ChevronRight size={17}/>
    </button>
    <div className="real-sheet-quick">
      {isDiscrete(target) && !usesPartialIntervals(target) ? quickCodes.map((code) => <button type="button" disabled={paused} className={code === "I" || code === "O" ? "independent" : code === "X" || code === "N" ? "incorrect" : "prompt"} key={code} onClick={() => onTrial(code)}>{code}</button>)
        : usesEventCount(target) ? <><button type="button" disabled={paused} onClick={() => onFrequency(1)}>+1</button><button type="button" disabled={paused} onClick={() => onFrequency(5)}>+5</button><button type="button" onClick={onOpen}>Reloj</button></>
...
export default function RealSessionCollector({ initialDraft, onClosed, onDiscarded, notify }: { initialDraft: CollectionDraft; onClosed: () => void; onDiscarded: () => void; notify: (message: string) => void }) {
  ...
  function recordTrialFor(target: CollectionTarget, code: TrialResponseCode) {
    ...
    const stepIndex = usesTaskAnalysis(target) ? active.length % steps.length : undefined;
    const probe = target.state === "baseline" || target.state === "maintenance";
    changeCapture(target, (capture) => ({ ...capture, opportunities: capture.opportunities + 1, observations: [...capture.observations, { id: crypto.randomUUID(), at: new Date().toISOString(), value: code === "I" || code === "O" ? 1 : 0, responseCode: code, ...(stepIndex !== undefined ? { taskStepIndex: stepIndex, taskStep: steps[stepIndex] } : {}), ...(probe ? { probe: true } : {}) }] }));
  }
  ...
  function finishInterval(occurred: boolean) {
    ...
    if (!allowed) return;
    changeCapture(focused, (capture) => ({ ...capture, opportunities: capture.opportunities + 1, observations: [...capture.observations, { id: crypto.randomUUID(), at: new Date().toISOString(), value: occurred ? 1 : 0, responseCode: occurred ? "O" : "N", intervalSeconds: focused.sessionConfig.intervalSeconds }] }));
    setIntervalState(null);
  }
  ...
  const focusedMeasurement = normalizeMeasurementConfig(focused);
  const focusedPromptLevels = usesPromptLevels(focused);
  const focusedBinary = isBinaryOpportunityMeasurement(focused);
  const focusedInterval = usesPartialIntervals(focused);
  const focusedEventCount = usesEventCount(focused);
  const focusedTimed = usesTimedMeasurement(focused);
  ...
  return <ModalLayer className="real-session-overlay" role="dialog" aria-modal="true" aria-label="Toma de sesión" onDismiss={() => dataPresent ? setExitOpen(true) : discardEmpty()} dismissDisabled={closing || closeSubmitted || discarding}>
    ...
    <div className="real-save-feedback" role="alert" hidden={saveState !== "error"}>{saveError || "No se confirmó el guardado."} Los datos siguen en esta ventana.<button onClick={() => persist(draftRef.current)}>Reintentar guardado</button></div><div className="real-session-main"><aside className="real-target-rail"><header><strong>Targets de hoy</strong><small>Último registro · muestra</small></header>{targets.map((target) => { const capture = draft.captures[target.id]; const status = targetStatus(target, capture); const currentLast = activeObservations(capture || targetCapture(target)).at(-1)?.responseCode || target.lastPromptCode; const promptLevels = usesPromptLevels(target); return <button className={`${target.id === focused.id ? "active" : ""} ${status.minimumMet ? status.evaluation.status === "met" ? "sample-met" : "sample-ready" : ""}`} key={target.id} onClick={() => selectTarget(target.id)}><span className={`target-state ${target.state}`}>{targetStateLabel(target.state)}</span><strong>{target.code} · {target.name}</strong><small>{promptLevels ? "Ayuda" : "Registro"}: {responseLabel(target, currentLast)}</small><em>{resultText(target, capture)}</em>{status.minimumMet && <i>{status.evaluation.status === "met" ? "Muestra y criterio" : "Muestra completa"}</i>}</button>; })}</aside>
      ...
      {view === "sheet" ? <main className="real-sheet"><header><div><p className="section-kicker">Hoja de datos</p><h1>Todos los targets</h1></div><span>{targets.filter((target) => capturedResult(target, draft.captures[target.id]).sampled).length}/{targets.length} con datos</span></header><div>{targets.map((target) => <RealSheetRow key={target.id} target={target} capture={draft.captures[target.id]} paused={paused} onOpen={() => selectTarget(target.id)} onTrial={(code) => recordTrialFor(target, code)} onFrequency={(amount) => adjustFrequencyFor(target, amount)}/>)}</div></main> : <main className="real-take"><section className="real-target-heading"><div><span className={`target-state ${focused.state}`}>{targetStateLabel(focused.state)}</span><small>{focused.programName}</small><h1>{focused.code} · {focused.name}</h1><p><strong>Objetivo:</strong> {focused.specificObjective}</p><p className="real-sd"><strong>SD:</strong> {focused.sessionConfig.discriminativeStimulus || "No configurado"}</p></div><div className={`real-sample-badge ${focusedStatus.minimumMet ? focusedStatus.evaluation.status === "met" ? "met" : "ready" : ""}`}><small>Muestra mínima</small><strong>{focusedStatus.result.opportunities}/{focusedStatus.criterion?.minTrials || 0}</strong></div></section>
        ...
        <section className="real-capture-card"><header><div><small>Registro</small><strong>{measurementLabel(focused)}{probe ? " · Sonda" : ""}</strong></div><span>{resultText(focused, currentCapture)}</span></header>
          {focusedBinary && !focusedInterval && <div className="real-discrete"><div className="real-trial-metrics"><div><strong>{focusedStatus.result.correct || 0}/{focusedStatus.result.opportunities}</strong><small>{focusedPromptLevels ? "Independientes" : "Ocurrió"}</small></div><div><strong>{focusedStatus.result.value ?? 0}%</strong><small>{focusedPromptLevels ? "Resultado" : "Oportunidades"}</small></div><div><strong>{responseLabel(focused, lastResponse || focused.lastPromptCode)}</strong><small>{focusedPromptLevels ? "Última ayuda" : "Último registro"}</small></div></div>{usesTaskAnalysis(focused) && <div className="real-task-step"><span>{taskStepIndex + 1}/{steps.length}</span><div><small>Paso actual de la cadena</small><strong>{steps[taskStepIndex]}</strong></div></div>}<div className="real-trial-pad">{focusedPromptLevels ? TRIAL_RESPONSES.filter((item) => !probe || item.code === "I" || item.code === "X" || probePromptsOpen).map((item) => <button disabled={paused} className={item.tone} key={item.code} onClick={() => recordTrial(item.code)}><b>{item.code}</b><span>{item.label}</span></button>) : <><button disabled={paused} className="independent" onClick={() => recordTrial("O")}><b>O</b><span>Ocurrió</span></button><button disabled={paused} className="incorrect" onClick={() => recordTrial("N")}><b>N</b><span>No ocurrió</span></button></>}</div>{focusedPromptLevels && probe && !probePromptsOpen && <button className="real-probe-prompts" onClick={() => setProbePromptsOpen(true)}>Registrar una ayuda que ocurrió</button>}{focusedPromptLevels && lastResponse && <div className={`real-trial-guidance ${lastResponse === "X" ? "error" : lastResponse === "I" ? "independent" : "prompt"}`}>{lastResponse === "I" ? "Siga sin ayuda." : lastResponse === "X" ? "Corrija con el mínimo de ayuda y vuelva a presentar el SD sin ayuda." : "En el siguiente ensayo intente independiente o un nivel menos de ayuda."}</div>}<div className="real-trial-strip"><div>{activeTrials.slice(-20).map((event, index) => <span className={event.responseCode === "I" || event.responseCode === "O" ? "independent" : event.responseCode === "X" || event.responseCode === "N" ? "incorrect" : "prompt"} key={event.id}><small>{Math.max(1, activeTrials.length - 19 + index)}</small><strong>{responseGlyph(focused, event.responseCode, event.value)}</strong></span>)}</div><button disabled={!activeTrials.length} onClick={undoLast}><RotateCcw size={15}/> Deshacer</button></div></div>}
          {focusedEventCount && <div className="real-frequency"><div className="real-frequency-number"><small>Ocurrencias</small><strong>{activeTrials.reduce((sum, event) => sum + event.value, 0)}</strong><span>{frequencyObservationMs > 0 ? `${Math.round((activeTrials.reduce((sum, event) => sum + event.value, 0) / frequencyObservationMs) * 600000) / 10} por minuto` : "Inicia la observación"}</span></div><div className="real-frequency-actions"><button disabled={paused} onClick={() => adjustFrequency(1)}>+1</button><button disabled={paused} onClick={() => adjustFrequency(5)}>+5</button></div><button disabled={paused} className={`real-observation-clock ${currentCapture.frequencyObservationStartedAt ? "running" : ""}`} onClick={toggleFrequencyObservation}>{currentCapture.frequencyObservationStartedAt ? <Pause size={18}/> : <Play size={18}/>}<span><small>Observación de esta conducta</small><strong>{formatElapsed(frequencyObservationMs / 1000)}</strong></span></button></div>}
          {focusedTimed && <div className="real-duration"><Timer size={27}/><small>Acumulado del día</small><strong>{formatElapsed(measuredSeconds)}</strong><button disabled={paused} className={currentCapture.timerStartedAt ? "running" : ""} onClick={toggleMeasurementTimer}>{currentCapture.timerStartedAt ? <><Pause size={19}/> Detener y guardar</> : <><Play size={19}/> Iniciar {focusedMeasurement.measurementDimension === "duration" ? "duración" : "latencia"}</>}</button><span>{activeTrials.length} registro{activeTrials.length === 1 ? "" : "s"} guardado{activeTrials.length === 1 ? "" : "s"}</span></div>}
          {focusedInterval && <div className="real-interval"><div><small>Intervalo parcial</small><strong>{focused.sessionConfig.intervalSeconds} s</strong><span>{intervalState?.targetId === focused.id ? intervalExpired ? "Intervalo terminado" : `${Math.max(0, Math.ceil((intervalState.endsAt - nowMs) / 1000))} s restantes` : `${activeTrials.length} intervalos guardados`}</span></div>{!intervalState || intervalState.targetId !== focused.id ? <button disabled={paused} onClick={startInterval}><Play size={19}/> Iniciar intervalo</button> : intervalExpired ? <button className="did-not-occur" onClick={() => finishInterval(false)}><X size={19}/> No ocurrió</button> : <button className="occurred" onClick={() => finishInterval(true)}><Check size={19}/> Ocurrió</button>}<p>{intervalState?.targetId === focused.id && !intervalExpired ? "Durante el intervalo solo se puede marcar Ocurrió." : intervalExpired ? "Ahora puedes registrar No ocurrió." : "Inicia el reloj para observar."}</p></div>}
