...
export function RealSessionSetup({
  ...
  return <div className="modal-backdrop real-setup-backdrop"><section className="real-session-setup" role="dialog" aria-modal="true" aria-labelledby="real-session-setup-title">
    <header><div><p className="section-kicker">Antes de empezar</p><h2 id="real-session-setup-title">Preparar toma de sesión</h2>{preparation?.profile && <span>{preparation.profile.fullName} · {preparation.profile.site}</span>}</div><button aria-label="Cerrar" onClick={onClose}><X size={20}/></button></header>
    {loading || !preparation ? <div className="real-setup-loading"><LoaderCircle className="spin" size={27}/><strong>Revisando expediente, agenda y targets…</strong></div> : <div className="real-setup-content">
    ...
    <footer><button className="secondary-formation-button" onClick={onClose}>Cancelar</button><button className="primary-formation-button" disabled={loading || starting || !preparation || !selected.size || !value.contextCategory || (value.contextCategory === "Otro" && !value.contextOther.trim())} onClick={onStart}>{starting ? <LoaderCircle className="spin" size={17}/> : <Play size={17}/>} {starting ? "Abriendo…" : "Iniciar sesión"}</button></footer>
  </section></div>;
}
...
export default function RealSessionCollector({ initialDraft, onClosed, onDiscarded, notify }: { initialDraft: CollectionDraft; onClosed: () => void; onDiscarded: () => void; notify: (message: string) => void }) {
  ...
  useEffect(() => () => { alive.current = false; if (saveTimer.current) clearTimeout(saveTimer.current); }, []);

  function persist(next: CollectionDraft) {
    setSaveState("saving");
    saveQueue.current = saveQueue.current.catch(() => undefined).then(async () => {
      const response = await fetch("/api/session-collection", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ draft: next }) });
      const body = await response.json() as { error?: string };
      if (!response.ok) throw new Error(body.error || "No se confirmó el guardado.");
      if (alive.current) setSaveState("saved");
    }).catch((error: Error) => { if (alive.current) { setSaveState("error"); notify(error.message); } });
  }
  ...
  async function discardEmpty() {
    if (dataPresent) { setExitOpen(true); return; }
    const response = await fetch("/api/session-collection", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: draft.id }) });
    const body = await response.json() as { error?: string };
    if (!response.ok) return notify(body.error || "No se pudo descartar la sesión vacía.");
    onDiscarded();
  }
  ...
  async function closeSession() {
    ...
    try {
      finalDraft = closeCollectionDraft({ ...current, signature: therapistSignature, closing: { ...current.closing!, coordinatorSignature } }, at, () => crypto.randomUUID());
    } catch (error) {
    ...
    setClosing(true);
    try {
      const response = await fetch("/api/session-collection", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "close", payload: collectionPayload(finalDraft) }) });
    ...
    } catch (error) { setCloseError(error instanceof Error ? error.message : "No se pudo cerrar la sesión. El borrador permanece guardado."); }
  ...
  const noteComplete = draft.template.fields.every((field) => !field.required || draft.noteValues[field.id]?.trim());
  const closingFieldsComplete = Boolean(draft.closing && (!draft.closing.guardianPresent || draft.closing.guardianName.trim()) && (!draft.closing.concernPresent || draft.closing.concernNote.trim()));
  const intervalExpired = Boolean(intervalState && nowMs >= intervalState.endsAt);

  return <div className="real-session-overlay">
    <header className="real-session-bar"><button className="real-exit" onClick={() => dataPresent ? setExitOpen(true) : discardEmpty()}><ArrowLeft size={19}/><span>Salir</span></button><div className="real-session-person"><strong>{draft.preparation.profile.fullName}</strong><small>{draft.context} · {targets.length} targets</small></div><button className={`real-session-clock ${paused ? "paused" : ""}`} onClick={toggleSessionClock}>{paused ? <Play size={17}/> : <Pause size={17}/>}<span><small>{paused ? "En pausa" : "Tiempo de sesión"}</small><strong>{formatElapsed(sessionSeconds)}</strong></span></button><div className="real-view-switch" role="tablist"><button role="tab" aria-selected={view === "take"} className={view === "take" ? "active" : ""} onClick={() => setView("take")}><ClipboardList size={16}/>Tomar</button><button role="tab" aria-selected={view === "sheet"} className={view === "sheet" ? "active" : ""} onClick={() => setView("sheet")}><Grid3X3 size={16}/>Hoja</button></div><button className={`real-save-state ${saveState}`} type="button" disabled={saveState !== "error"} onClick={() => persist(draft)}><Save size={15}/><span>{saveState === "saving" ? "Guardando" : saveState === "error" ? "Reintentar guardado" : "Guardado"}</span></button>{draft.preparation.canRecordAbc && <button className="real-tool" onClick={openAbcCapture}><ListTree size={17}/><span>ABC</span>{draft.abc.length > 0 && <em>{draft.abc.length}</em>}</button>}<button className={`real-tool ${notesOpen ? "active" : ""}`} onClick={() => setNotesOpen(!notesOpen)}><NotebookPen size={17}/><span>Nota</span></button><button className="real-finish" onClick={() => setCloseOpen(true)}><CheckCircle2 size={17}/>Cerrar</button></header>
    ...
    <div className="real-session-main"><aside className="real-target-rail"><header><strong>Targets de hoy</strong><small>Última ayuda · muestra</small></header>{targets.map((target) => { const capture = draft.captures[target.id]; const status = targetStatus(target, capture); const currentLast = activeObservations(capture || targetCapture(target)).at(-1)?.responseCode || target.lastPromptCode; return <button className={`${target.id === focused.id ? "active" : ""} ${status.minimumMet ? status.evaluation.status === "met" ? "sample-met" : "sample-ready" : ""}`} key={target.id} onClick={() => selectTarget(target.id)}><span className={`target-state ${target.state}`}>{targetStateLabel(target.state)}</span><strong>{target.code} · {target.name}</strong><small>Ayuda: {responseLabel(currentLast)}</small><em>{resultText(target, capture)}</em>{status.minimumMet && <i>{status.evaluation.status === "met" ? "Muestra y criterio" : "Muestra completa"}</i>}</button>; })}</aside>
      ...
      {notesOpen && <aside className="real-notes"><header><div><small>Se guarda al cerrar</small><strong>{draft.template.name}</strong></div><button aria-label="Cerrar nota" onClick={() => setNotesOpen(false)}><X size={18}/></button></header><p>{draft.template.description}</p><div>{draft.template.fields.map((field) => <label key={field.id}><span>{field.label}{field.required && <em>Obligatorio</em>}</span>{field.guidance && <small>{field.guidance}</small>}<textarea value={draft.noteValues[field.id] || ""} onChange={(event) => updateDraft((current) => ({ ...current, noteValues: { ...current.noteValues, [field.id]: event.target.value } }), false)} placeholder="Información observable…"/></label>)}</div></aside>}
    </div>

    {abcOpen && <div className="real-session-modal"><section className="real-abc-modal" role="dialog" aria-modal="true" aria-labelledby="real-abc-title"><header><div><p className="section-kicker">Sin salir de la sesión</p><h2 id="real-abc-title">Registrar ABC</h2></div><button onClick={() => setAbcOpen(false)} aria-label="Cerrar"><X size={19}/></button></header><div className="real-abc-grid"><label><span>Antecedente</span><textarea autoFocus value={abcDraft.antecedent} onChange={(event) => setAbcDraft({ ...abcDraft, antecedent: event.target.value })} placeholder="Qué ocurrió inmediatamente antes"/></label><label><span>Conducta</span><textarea value={abcDraft.behavior} onChange={(event) => setAbcDraft({ ...abcDraft, behavior: event.target.value })} placeholder="Qué hizo exactamente el niño"/></label><label><span>Consecuencia</span><textarea value={abcDraft.consequence} onChange={(event) => setAbcDraft({ ...abcDraft, consequence: event.target.value })} placeholder="Qué ocurrió inmediatamente después"/></label><label><span>Intensidad</span><select value={abcDraft.intensity} onChange={(event) => setAbcDraft({ ...abcDraft, intensity: Number(event.target.value) })}>{[1,2,3,4,5].map((value) => <option value={value} key={value}>{value}{value === 1 ? " · mínima" : value === 5 ? " · máxima" : ""}</option>)}</select></label><label><span>Actividad</span><input value={abcDraft.activity} onChange={(event) => setAbcDraft({ ...abcDraft, activity: event.target.value })} placeholder="Actividad en curso"/></label><label><span>Target relacionado</span><select value={abcDraft.targetId} onChange={(event) => { const target = targets.find((item) => item.id === event.target.value); setAbcDraft({ ...abcDraft, targetId: event.target.value, programId: target?.programId || "" }); }}><option value="">Sin target</option>{targets.map((target) => <option value={target.id} key={target.id}>{target.code} · {target.name}</option>)}</select></label><label className="wide"><span>Observación adicional</span><textarea value={abcDraft.note} onChange={(event) => setAbcDraft({ ...abcDraft, note: event.target.value })}/></label></div><footer><button className="secondary-formation-button" onClick={() => setAbcOpen(false)}>Cancelar</button><button className="primary-formation-button" onClick={saveAbc}><Save size={16}/> Agregar a la sesión</button></footer></section></div>}

    {exitOpen && <div className="real-session-modal"><section className="real-exit-modal" role="alertdialog" aria-modal="true"><ShieldAlert size={27}/><h2>Esta sesión ya tiene datos</h2><p>No se puede descartar. El borrador está guardado; para salir debes cerrar y firmar la sesión.</p><div><button className="secondary-formation-button" onClick={() => setExitOpen(false)}>Continuar tomando</button><button className="primary-formation-button" onClick={() => { setExitOpen(false); setCloseOpen(true); }}>Ir al cierre</button></div></section></div>}

    {closeOpen && <div className="real-session-modal close"><section className="real-close-modal" role="dialog" aria-modal="true" aria-labelledby="real-close-title"><header><div><p className="section-kicker">Etapa final</p><h2 id="real-close-title">Cerrar sesión</h2><span>{formatElapsed(sessionSeconds)} · {draft.preparation.profile.fullName}</span></div><button aria-label="Volver" onClick={() => setCloseOpen(false)}><X size={20}/></button></header><div className="real-close-content"><section className="real-close-summary"><h3>Resumen por target</h3><div>{targets.map((target) => { const status = targetStatus(target, draft.captures[target.id]); const label = !status.result.sampled ? "Sin dato" : status.evaluation.status === "met" ? "Cumple" : status.evaluation.status === "not_met" ? "No cumple" : status.evaluation.status === "insufficient_sample" ? "Muestra corta" : "Sin dato"; return <article key={target.id}><div><strong>{target.code} · {target.name}</strong><small>{resultText(target, draft.captures[target.id])}</small></div><span className={status.evaluation.status}>{label}</span></article>; })}</div></section><section className="real-close-note"><h3>Nota de sesión</h3>{draft.template.fields.map((field) => <label key={field.id}><span>{field.label}{field.required && <em>Obligatorio</em>}</span><textarea value={draft.noteValues[field.id] || ""} onChange={(event) => updateDraft((current) => ({ ...current, noteValues: { ...current.noteValues, [field.id]: event.target.value } }), false)}/></label>)}</section><section className="real-close-fields"><h3>Actividades e incidencias</h3><label><span>Actividades realizadas</span><textarea value={draft.closing?.activities || ""} onChange={(event) => updateDraft((current) => ({ ...current, closing: { ...current.closing!, activities: event.target.value } }), false)}/></label><label><span>Incidentes</span><textarea value={draft.closing?.incidents || ""} onChange={(event) => updateDraft((current) => ({ ...current, closing: { ...current.closing!, incidents: event.target.value } }), false)} placeholder="Escribe los incidentes o indica que no hubo."/></label><fieldset><legend>¿Estuvo el tutor?</legend><label><input type="radio" checked={!draft.closing?.guardianPresent} onChange={() => updateDraft((current) => ({ ...current, closing: { ...current.closing!, guardianPresent: false, guardianName: "" } }), false)}/> No</label><label><input type="radio" checked={Boolean(draft.closing?.guardianPresent)} onChange={() => updateDraft((current) => ({ ...current, closing: { ...current.closing!, guardianPresent: true } }), false)}/> Sí</label>{draft.closing?.guardianPresent && <input value={draft.closing.guardianName} onChange={(event) => updateDraft((current) => ({ ...current, closing: { ...current.closing!, guardianName: event.target.value } }), false)} placeholder="Nombre del tutor"/>}</fieldset><fieldset><legend>¿Preocupación médica o ambiental?</legend><label><input type="radio" checked={!draft.closing?.concernPresent} onChange={() => updateDraft((current) => ({ ...current, closing: { ...current.closing!, concernPresent: false, concernNote: "" } }), false)}/> No</label><label><input type="radio" checked={Boolean(draft.closing?.concernPresent)} onChange={() => updateDraft((current) => ({ ...current, closing: { ...current.closing!, concernPresent: true } }), false)}/> Sí</label>{draft.closing?.concernPresent && <textarea value={draft.closing.concernNote} onChange={(event) => updateDraft((current) => ({ ...current, closing: { ...current.closing!, concernNote: event.target.value } }), false)} placeholder="Describe obligatoriamente la preocupación"/>}</fieldset></section><section className="real-signatures"><h3>Firmas</h3><div><label><span>Terapeuta · obligatorio</span><strong>{draft.preparation.professionalName || "Profesional de la sesión"}</strong><SignaturePad strokes={therapistStrokes} onChange={setTherapistStrokes} label="Firma del terapeuta"/></label><label><span>Coordinador · opcional</span><input value={draft.closing?.coordinatorName || ""} onChange={(event) => updateDraft((current) => ({ ...current, closing: { ...current.closing!, coordinatorName: event.target.value } }), false)} placeholder="Nombre del coordinador"/><SignaturePad strokes={coordinatorStrokes} onChange={setCoordinatorStrokes} label="Firma opcional del coordinador"/></label></div></section>{closeError && <div className="real-close-error"><AlertTriangle size={17}/>{closeError}</div>}</div><footer>{!dataPresent ? <button className="danger-button" onClick={discardEmpty}><Trash2 size={16}/> Descartar sesión vacía</button> : <span><Save size={15}/> El borrador clínico no puede descartarse.</span>}<div><button className="secondary-formation-button" onClick={() => setCloseOpen(false)}>Volver a la toma</button><button className="primary-formation-button" disabled={closing || !dataPresent || !noteComplete || !closingFieldsComplete || !therapistStrokes.length} onClick={closeSession}>{closing ? <LoaderCircle className="spin" size={17}/> : <CheckCircle2 size={17}/>} {closing ? "Cerrando…" : "Firmar y cerrar"}</button></div></footer></section></div>}
  </div>;
}
...
function SignaturePad({ strokes, onChange, label }: { strokes: SignaturePoint[][]; onChange: Dispatch<SetStateAction<SignaturePoint[][]>>; label: string }) {
  ...
  function start(event: ReactPointerEvent<SVGSVGElement>) { drawing.current = true; event.currentTarget.setPointerCapture(event.pointerId); const next = point(event); onChange((current) => [...current, [next]]); }
