...
const appointment = { id: "qa-appointment", professionalRole: "terapeuta" as const, cancellationCategory: null, cancellationReason: "", cancelledByAccountId: null, cancelledAt: null, profileId: profile.id, profileName: profile.fullName, professionalAccountId: "qa-professional", professionalName: "Profesional QA", site: "León", sessionDate: params.has("past") ? "2026-09-04" : today, startTime: "09:00", endTime: "10:00", sessionType: "Prueba aislada", notes: "", status: "scheduled" as const, canStart: true, hasClinicalEvidence: false, clinicalSessionRunId: null, interventionSessionId: null };
...
function Harness() {
  ...
  const [initialAppointment, setInitialAppointment] = useState<typeof appointment | null>(params.has("openAppointment") ? appointment : null);
  const consumeAppointment = useCallback(() => { if (!params.has("openAppointment")) setInitialAppointment(null); }, []);
  return <><div className="formation-shell"><aside className="formation-sidebar"><button onClick={() => setScreen("calendar")}>Calendario QA</button><button onClick={() => setScreen("sessions")}>Sesiones QA</button></aside><section className="formation-workspace"><header className="formation-topbar"><strong>QA aislada · Sin datos reales</strong><label>Niño<select><option>Caso ficticio QA</option></select></label></header><main className="formation-content">
    ...
    {screen === "graphs" ? <GraphManager cycles={[]} profiles={[profile]} selectedProfileId={profile.id} notify={setNotice} onOpenABC={()=>setScreen("abc")}/> : screen === "abc" ? <ABCManager profileId={profile.id} profileName={profile.fullName} notify={setNotice}/> : screen === "calendar" ? <CalendarManager compact={params.has("compact")} profiles={[profile]} notify={setNotice} onOpenSession={(a) => { setInitialAppointment(a as typeof appointment); setScreen("sessions"); }}/> : screen === "programs" ? <InterventionSessionManager mode="programs" profiles={[profile]} cycles={[]} selectedProfileId={profile.id} selectedSite="Todas" onSelectProfile={() => undefined} onProfilesRefresh={() => undefined} notify={setNotice} canManagePrograms canRecordSessions canManageSessions draftOwnerId="qa-audit"/> : <InterventionSessionManager mode="sessions" profiles={[profile]} cycles={[]} selectedProfileId="all" selectedSite="Todas" onSelectProfile={() => undefined} onProfilesRefresh={() => undefined} notify={setNotice} canManagePrograms canRecordSessions canManageSessions allowAdHocSessions initialAppointment={initialAppointment} onAppointmentConsumed={consumeAppointment} draftOwnerId="qa-audit"/>}
